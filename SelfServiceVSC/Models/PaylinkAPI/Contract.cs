...
namespace AAC.SelfServiceVSC.Models.PaylinkAPI
{
	public class Contract
	{
		...
		[JsonIgnore(Condition = JsonIgnoreCondition.WhenWritingNull)]
		public String PPAPostbackURL { get; set; }
			...
			SessionDetails sessionDetails)
		{
			...
			//var downPayment = Math.Round(Decimal.Parse(sessionDetails.FinalizeContract.Downpayment) * totalRate, 0);
			//var downPaymentCharge = Math.Max(5m, downPayment * 0.05m);
			int term = 0;
			Int32.TryParse(sessionDetails.FinalizeContract.Term, out term);
			...
			var dealerCost = Math.Min(Math.Floor(totalRate * fee.FeeRate), totalRate - fee.FeeFlat);
			...
			FirstPaymentDate = DateTime.Now.AddMonths(1).ToShortDateString();
